...
if (!class_exists('JAImage')) {
    ...
    class JAImage
    {
        ...
        function parseImage($row)
        {
			...
			if((isset($images->video_fulltext) and !empty($images->video_fulltext)) || (isset($images->image_fulltext) and !empty($images->image_fulltext)) || (isset($images->image_intro) and !empty($images->image_intro))){
				$image = (isset($images->video_fulltext) and !empty($images->video_fulltext))?$images->video_fulltext:(isset($images->image_intro) and !empty($images->image_intro))?$images->image_intro:((isset($images->image_fulltext) and !empty($images->image_fulltext))?$images->image_fulltext:"");
			}
			else {
				$regex = '/\<img.+src\s*=\s*\"([^\"]*)\"[^\>]*\>/';
				$text = '';
